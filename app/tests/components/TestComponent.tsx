  ...
  return (
    <div className="w-full lg:w-4/5 flex flex-col">
      ...
      >
        ...
        <span className="flex items-center">{testInfo.name}</span>
      </div>
      {open && (
        <div className="w-full h-fit p-1 pl-8 lg:pl-12 m-1 text-sm lg:text-base flex items-center  text-white rounded-md text-slate-200">
          <span className="flex items-center">{testInfo.content}</span>
        </div>
      )}
    </div>
  );
